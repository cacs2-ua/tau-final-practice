...
def _feature_names_after_preprocessing(pre_fitted: Pipeline) -> np.ndarray:
    ...
    # transformers_ exists only after fit()
    for name, trans, cols in getattr(ct, "transformers_", []):
        ...
        if name == "num":
            ...
            # our cat transformer is a Pipeline(..., ("onehot", OneHotEncoder))
            ...
            # fallback: assume 1 feature per input column
...
def _build_interpretation_tables(
    ...
    *,
    ...
    # --- support + importance aligned to preprocessed feature space ---
    support = None
    importance = None
    ...
    # ---- CRITICAL GUARD (prevents your “arrays must be same length” crash) ----
    ...
    # Base columns = exactly what ColumnTransformer saw
